	...
	.then(res =>{
		if(res.ok){
			...
			res.json().then(clima => {
				...
				if(tempImg > 25){
					message.innerHTML=(`${localidad.name}, ${localidad.province} <img src="static/img/high_temp.png" alt="Cuidado! la temperatura es alta"> <strong>${localidad.weather.tempDesc}</strong> ${localidad.weather.description}`);                   
					//Si la temperatura es menor a 10
				}else if (tempImg < 10){
					message.innerHTML=(`${localidad.name}, ${localidad.province} <img src="static/img/low_temp.png" alt="Cuidado! la temperatura es baja"> <strong>${localidad.weather.tempDesc}</strong> ${localidad.weather.description}`);
				}else{
					message.innerHTML=(`${localidad.name}, ${localidad.province} <img src="static/img/medium_temp.png" alt="La temperatura es agradable"> <strong>${localidad.weather.tempDesc}</strong> ${localidad.weather.description}`);
				}
